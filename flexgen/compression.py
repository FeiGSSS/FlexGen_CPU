...
from flexgen.torch_backend import (TorchTensor,
                                   TorchDevice,
                                   DeviceType,
                                   general_copy,
...
class TorchCompressedDevice:
    ...
    def __init__(self, base_device: TorchDevice):
        self.name = "compressed"
        self.DeviceType = DeviceType.COMPRESSED
    ...
    def allocate(self,
        ...
        assert comp_config.num_bits == 4 and dtype == np.float32, "dtype = ".format(dtype)
        ...
        data = self.base_device.allocate(data_shape, np.uint8)
        scale = self.base_device.allocate(scale_shape, np.float16)

        from flexgen.utils import np_dtype_to_torch_dtype
        return TorchTensor(shape, np_dtype_to_torch_dtype[dtype],
                           (data, scale, comp_config), self, name=name)

    def init_cache_one_batch(self, config:OptConfig, task, policy):
        num_head, hidden_size, prompt_len, gen_len, batch_size = (
            config.n_head, config.input_dim, task.prompt_len, task.gen_len,
            policy.batch_size)
        shape = (prompt_len + gen_len - 1, batch_size * num_head, hidden_size // num_head)
        # NOTE: disable pin_memory due to high memory overhead
        k_cache = self.allocate(shape, np.float32, comp_config=policy.comp_cache_config)
        v_cache = self.allocate(shape, np.float32, comp_config=policy.comp_cache_config)
    ...
    def init_attention_compute_workspace(self,
        ...
        group_size = policy.comp_cache_config.group_size
        group_dim = policy.comp_cache_config.group_dim
        
        num_groups = (shape[group_dim] + group_size - 1) // group_size
        new_shape = (shape[:group_dim] + (num_groups, group_size) + shape[group_dim+1:])

        self.data_decompress_workspace = [
            torch.empty(*new_shape, dtype=torch.float32, device=self.base_device.dev),
            torch.empty(*new_shape, dtype=torch.float32, device=self.base_device.dev),
        ]

    def compress(self, tensor:torch.Tensor, comp_config:CompressionConfig):
        ...
        # Quantize
        B = 2 ** num_bits - 1 # max value for 4-bit quantization
    ...
    def decompress(self, tensor):
        ...
        # Unpack
        self.workspace_pt = (self.workspace_pt + 1) % len(self.data_decompress_workspace)
...
def general_copy_compressed(dst, dst_indices, src, src_indices):
    assert (src.device.DeviceType == DeviceType.COMPRESSED and
